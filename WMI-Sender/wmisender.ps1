...
function fncncentralwmisender {
    ...
    #function to create a namespace, class, instance and add properties to it.
        ...
        $WMI_Class.Properties.Add("Device_id", [System.Management.CimType]::String, $false) 
        $WMI_Class.Properties["Device_id"].Qualifiers.Add("key", $true) 
    ...
    function fncInstance_Update{
        ...
        fnclog "Update instance - Update instance with data in instance $($namespace)"
        $instancetoupdate = Get-CimInstance -Namespace $($namespace) -ClassName $($NCentral_servicename) | Where-Object "Device_id" -eq "$NCentral_device_id"
    ...
    $namespace = "$($Ncentral_wmi_namespace)\$($NCentral_servicename)"
    ...
    $Simclassdetect = Get-CimInstance -Namespace $($namespace) -ClassName $($NCentral_servicename) -ErrorAction Ignore | Where-Object "Device_id" -eq "$NCentral_device_id" 
    if($Simclassdetect){
